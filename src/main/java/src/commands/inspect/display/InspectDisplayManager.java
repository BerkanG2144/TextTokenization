...
public class InspectDisplayManager {
    ...
    /**
     * Displays the match context with underlining and proper alignment.
     *
    ...
    private void displayMatchContext(MatchContextData contextData) {
        ...
        // Berechne Präfixlängen für Ausrichtung
        int contextStart1 = Math.max(0, startChar1 - contextData.getContextSize());
        int contextStart2 = Math.max(0, startChar2 - contextData.getContextSize());

        int prefix1Length = (contextStart1 > 0 ? 3 : 0) + (startChar1 - contextStart1);
        int prefix2Length = (contextStart2 > 0 ? 3 : 0) + (startChar2 - contextStart2);

        // Berechne Ausrichtungs-Leerzeichen
        int alignmentSpaces1 = Math.max(0, prefix2Length - prefix1Length);
        int alignmentSpaces2 = Math.max(0, prefix1Length - prefix2Length);

        // Gebe ausgerichtete Kontexte aus
        System.out.println(" ".repeat(alignmentSpaces1) + context1);
        printUnderline(contextData.getFirstText(), startChar1, endChar1, contextData.getContextSize(), alignmentSpaces1);
        System.out.println(" ".repeat(alignmentSpaces2) + context2);
        printUnderline(contextData.getSecondText(), startChar2, endChar2, contextData.getContextSize(), alignmentSpaces2);
    }
    ...
    /**
     * Prints underline for matched text with proper alignment.
     *
     ...
     * @param alignmentSpaces additional alignment spaces
     */
    private void printUnderline(String text, int startChar, int endChar, int contextSize, int alignmentSpaces) {
        ...
        // Füge Ausrichtungs-Leerzeichen hinzu
        for (int i = 0; i < alignmentSpaces; i++) {
            underline.append(" ");
        }

        // Füge Kontext-Leerzeichen hinzu
        for (int i = 0; i < matchStartInContext; i++) {
            underline.append(" ");
        }

        // Füge Unterlinierung hinzu
